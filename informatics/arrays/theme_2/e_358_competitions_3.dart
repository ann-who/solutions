  ...
  for (var sportsmen = 0; sportsmen < rowsAndColumns[0]; sportsmen++) {
    ...
    for (var column = 0; column < matrix[sportsmen].length; column++) {
      ...
      if (sportsmenTry > bestTry) {
        ...
        int currentSportsmenSum = 0;
        int bestSportsmenSum = 0;
        for (var point in matrix[sportsmen]) {
          currentSportsmenSum += point;
        }
        for (var point in matrix[bestSportsmen]) {
          bestSportsmenSum += point;
        }

        if (currentSportsmenSum > bestSportsmenSum) {
          bestSportsmen = sportsmen;
        }
      }
    }
